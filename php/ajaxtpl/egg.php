...
if($pn==39&&$obj->prizenumber==''){
	...
	//消息通知获奖用户
	create_openid_prize_message($user->openid,$prizenumber,"http://cha.gdbwt.com/egg_address.php");
	if($obj->openid!='0'){
	...
	/**	$db->query("update egg_number set prizenumber='".($obj->number.",uid:".$prizenumber2)."',updateTime='".time()."' where openid = '".$obj->openid."'");*/
		//消息通知邀请者同时获奖用户
		create_openid_prize_message($obj->openid,$prizenumber2,"http://cha.gdbwt.com/egg_address.php");
	}
}
