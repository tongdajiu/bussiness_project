 ...
 * @param string $url
 */
function redirect($url = '', $msg = '', $isFrame = FALSE)
{
	if($url == '')
	{
		$url = 'index.php';
	...
	else
	{
		if( $msg != ''  )
		{
			$strs = '<script type="text/javascript">alert("' . $msg . '");top.location.href="' . $url . '";</script>';
		}
		else
		{
			$strs = '<script language="javascript">top.location.href = "' . $url . '";</script>';
		}
	}
...
function UploadOutputJson($uploader = 'file', $path = '../upfiles/') {
	$return = '';
	...
	if (count($_FILES) == 0)
	{
		echo get_json_data_public( '-1', '无上传文件！', '' );
		return;
	}
	...
	if($_SERVER['REQUEST_METHOD']=='POST' || $_SERVER['REQUEST_METHOD']=='GET'  )
	{
		...
		{
			echo get_json_data_public( '-1', '文件不存在！', '' );
			return;
		}
		...
		if( ! in_array($file['type'], $uptypes))
		{
			echo get_json_data_public( '-1', '上传文件格式不对！' .$file['type'], '' );
			return;
		}
		...
		echo get_json_data_public( '1', '上传文件成功！', $return );
		return;
	}
	else
	{
		echo get_json_data_public( '-1', '上传姿势不对！', '' );
		return;
	}
...
function renderPic($pic, $sizetype='small', $type=1, $size=array(), $info=array(), $echo=true){
	...
	if(empty($pic)){
		...
		//用于兼容前期与后期传递的参数的类型
		$typeMap = array(1=>'product', 2=>'logo', 3=>'productimage',4=>'storeImage',5=>'integralProductImage',6=>'article');
		$picType = is_int($type) ? $typeMap[$type] : $type;

		$types = array(1=>'product/');
		switch($picType){
			case 'product':
				$pics = '/upfiles/product/';
				switch($sizetype){
					case 'small':
						$pics .= 'small/';
						break;
			...
			case 'integralProductImage':
				$pics = '/upfiles/integral/';
				switch($sizetype){
					case 'small':
						$pics .= 'small/';
						break;
...
function M( $dbName, $db='' )
{
	global $db;
	static $_model  = array();
	require_once LIB_DIR . '/Model.class.php';
 ...
 *	功能：判断是否允许访问
 */
function isView( $arrUserRules )
{
	$func  	= !isset($_REQUEST['act']) ? 'home' :  $_REQUEST['module'];
	$act 	= !isset($_REQUEST['act']) ? 'list' :  $_REQUEST['act'];

	if ( $act == 'add_save' || $act == 'post' )
	{
		$act = 'add';
	}

	if ( $act == 'edit_save' || $act == 'update_status' )
	{
		$act = 'edit';
	}
	...
	$arrWhere = array( 'functions'=> $func, 'actions'=>$act );

	$AdminMap = M('admin_map');
	$rs = $AdminMap->get($arrWhere, array('id'));
	...
	if ( in_array( $rs->id, $arrUserRules ) )
	{
		return TRUE;
	}
	else
 ...
 *	功能：获取系统中所有功能的ID编号
 */
function getAllFuncID()
{
	$AdminMap = M('admin_map');
	$arrFatherData  = $AdminMap->getAll(array('pid'=>0), '', 'OBJECT', '',array('id','name','functions'));

	foreach( $arrFatherData as $arrSubData )
	{
		$data[$arrSubData->id] = $arrSubData;
		$arrChildData  = $AdminMap->getAll(array('pid'=>$arrSubData->id), '', 'OBJECT', '',array('id','name','functions'));
		$data[$arrSubData->id]->child = $arrChildData;
	}
...
function ajaxResponse($state, $msg, $data=array()){
	$info = array('state'=>$state, 'msg'=>$msg, 'data'=>$data);
...
/* 保存微信文件 */
function w_log($filename, $filecontent)
{
	file_put_contents( LOG_DIR . $filename . '.log', $filecontent, FILE_APPEND);
}
