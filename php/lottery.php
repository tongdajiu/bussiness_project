...
empty ($_SESSION['userInfo']) && redirect('/login.php?dir=lottery.php');
...
$lotInfo = $Lottery->getIngGameInfo(1);									// 获取正在进行中的活动信息
$settingInfo   = $Lottery->getGamePrizeList( '', $lotInfo->lottery_id );    // 获取游戏的奖品信息
$arrLotteryImg = (object)json_decode($lotInfo->lottery_image);				// 获取游戏的图片
...
switch ($action)
{
	case 'check':
		...
		elseif ($lotInfo->start_time > time())
		{
			ajaxResponse( false, '活动尚未开始' );
		}
		elseif ($lotInfo->end_time < time())
		{
...
//			break;
		...
		addData( $user->id, $getPrizeInfo->prize, $getPrizeInfo->prize_type, $prize_val, $lotInfo->lottery_id  );
	...
	default : //界面
		include TEMPLATE_DIR.'/lottery_web.php';
}
