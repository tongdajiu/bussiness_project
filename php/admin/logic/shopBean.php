...
class shopBean
{
	...
	function updateNew($db,$arrParam,$arrCondition)
	{
		...
		if ( is_array($arrParam) )
		{
			...
			$strSQL = "UPDATE ".SHOP_TABLE." SET {$strVal} WHERE 1=1 {$strWhere}";
			$rs =  $db->query($strSQL);
		}
	...
	function createNew( $db, $arr )
	{
		...
		if ( is_array($arr) )
		{
			...
			$strSQL = "INSERT INTO ".SHOP_TABLE." ($strKey) VALUES({$strVal})";
	...
	function get_list( $db, $arrWhere, $arrCol = '', $strOrderBy = '' )
	{
		...
		$strSQL = "SELECT {$strCol} FROM ".SHOP_TABLE." WHERE 1=1 {$strWhere} {$OrderBy}";
	...
	function del( $db, $arrWhere )
	{
		...
		$strSQL = "DELETE FROM ".SHOP_TABLE." WHERE 1=1 {$strWhere} ";
