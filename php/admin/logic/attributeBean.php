...
class attributeBean
{
	...
	function updateNew($db,$arrParam,$arrCondition)
	{
		...
		if ( is_array($arrParam) )
		{
			...
			$strSQL = "UPDATE ".ATTRIBUTE_TABLE." SET {$strVal} WHERE 1=1 {$strWhere}";
			$rs =  $db->query($strSQL);
		}
	...
	function createNew( $db, $arr )
	{
		...
		if ( is_array($arr) )
		{
			...
			$strSQL = "INSERT INTO ".ATTRIBUTE_TABLE." ($strKey) VALUES({$strVal})";
	...
	function get_list( $db, $arrWhere = '', $arrCol = '', $strOrderBy = '' )
	{
		...
		$strSQL = "SELECT {$strCol} FROM ".ATTRIBUTE_TABLE." WHERE 1=1 {$strWhere} {$OrderBy}";
	...
	function del( $db, $arrWhere )
	{
		...
		$strSQL = "DELETE FROM ".ATTRIBUTE_TABLE." WHERE 1=1 {$strWhere} ";
	...
	{
		$db->query("delete from ".ATTRIBUTE_TABLE." where attr_id in (".implode(",",$id).")");
		return true;
	}
