...
define('PRODUCT_ATTR_TABLE',"product_attr");
class product_attrBean
{
	/** 更新数据
	 *
	...
	function updateNew($db,$arrParam,$arrCondition)
	{
		...
		if ( is_array($arrParam) )
		{
			...
			{
				$arrVal2[] = " AND `{$key}`='" . mysql_real_escape_string($val) . "'";
				$strWhere = implode('',$arrVal2);
			}

			$strSQL = "UPDATE ".PRODUCT_ATTR_TABLE." SET {$strVal} WHERE 1=1 {$strWhere}";
			$rs =  $db->query($strSQL);
		}
	...
	function createNew( $db, $arr )
	{
		...
		if ( is_array($arr) )
		{
			...
			$strSQL = "INSERT INTO ".PRODUCT_ATTR_TABLE." ($strKey) VALUES({$strVal})";
	...
	function get_list( $db, $arrWhere, $arrCol = '', $strOrderBy = '' )
	{
		...
		$strSQL = "SELECT {$strCol} FROM ".PRODUCT_ATTR_TABLE." WHERE 1=1 {$strWhere} {$OrderBy}";
	...
	function del( $db, $arrWhere )
	{
		...
		$strSQL = "DELETE FROM ".PRODUCT_ATTR_TABLE." WHERE 1=1 {$strWhere} ";
